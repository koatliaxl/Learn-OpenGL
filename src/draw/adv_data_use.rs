...
pub unsafe fn print_gl_f32_buffer(target: gl::types::GLenum, size: usize, format: usize) {
    #[allow(unused_imports)]
    ...
    let result = gl::UnmapBuffer(target);
    if result == gl::TRUE {
        println!("\nGL data was mapped successfully");
    } else {
        println!("\nGL data mapping was unsuccessful");
